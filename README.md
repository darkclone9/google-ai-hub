...
- From the same connector popup, **Add note from vault** turns the chosen Markdown note into a child tab beneath the source card's active Google Doc tab, copies its Markdown formatting into Google Docs, and replaces the Canvas node with the full editable `.gdoc` interface. If the source is already at Google Docs' deepest tab level, choose **Lift branch and nest** to move its containing branch up one level while keeping that branch together, or **Keep beside** to avoid changing the existing hierarchy. The same depth-safe choice appears when adding or nesting tabs from the tab menu. The original Markdown file remains in the vault as a safety copy.
...
If a tab drag previously produced an invisible empty node or dangling connector, reload the plugin. Google AI Hub makes one repair attempt per loaded Canvas, preserving the card position and existing edges without repeatedly reacting to its own DOM updates. A malformed card containing typed text is deliberately left unchanged so the unsynced draft cannot be discarded; copy that text before deleting and recreating the card.
